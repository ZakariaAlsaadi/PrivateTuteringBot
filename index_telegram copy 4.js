...
app.use(express.static(`C:\\saketdesktop\\Private Tutering Web Page\\`));
const port = 4040;
var token = [];
token[0] = "5232088474:AAHHrA2KbFkHH5VhrOn68QR-rDwxdECLU3o";
const apiUrl = `https://api.telegram.org/bot${token[0]}`;
const storagePath = `C:\\saketdesktop\\Private Tutering Web Page\\StorageOfAllPpl\\`;

const myServerUrl = `https://7dff-188-247-9-81.ngrok-free.app/`;
...
function searchingForListOfTeachers(message, userIndex) {
  ...
  if (userSearchingStep == 1) {
    ...
    if (message.text == "الغاء البحث") {
      resetSearching(message,"تم الغاء البحث بنجاح")
    } else {
      fs.readFile(
        `${storagePath}search${message.from.id}.json`,
        "utf8",
        (err, data) => {
          ...
          writeFile(searchingBox, `search${message.from.id}`);
          postTeachers(message,userIndex);
        }
      );
    }
  } else {
    resetSearching(message,"حدث خطأ حاول البحث مجدداً")
  }
}
...
app.get("/", (req, res) => {
  res.sendFile(
    `C:\\saketdesktop\\Private Tutering Web Page\\PrivateTutoring.html`
  );
});
app.get("/SecretBotChatWindow", (req, res) => {
  res.sendFile(
    `C:\\saketdesktop\\Private Tutering Web Page\\SecretBotChatWindow.html`
  );
...
app.get("/sarob", (req, res) => {
  res.sendFile(`C:\\saketdesktop\\Private Tutering Web Page\\Sarob.html`);
});
///////////////////////////////////////////////////////////////////////////////////////////////
function resetSearching (message,botReply) {
...
function deleteJsonFile(filePath) {
  fs.unlink(`${storagePath}${filePath}`, (err) => {
    if (err) {
      console.error(err);
      return;
    }
...
function postTeachers(message,userIndex) {
  fs.readFile(
    `${storagePath}search${message.from.id}.json`,
    "utf8",
    (err, data) => {
                ...
                if (
                  ...
                  if (
                    ...
                    if (
                      ...
                      sendMsg(
                        `@${storageInfo[i].userName} ${teacherInfoSlot.firstName} ${teacherInfoSlot.lastName} , ${teacherInfoSlot.priceList[0]} - ${teacherInfoSlot.priceList[1]} ليرة سورية`,
                        message.from.id
                      );
                      sendMsg(`${teacherInfoSlot.phoneNumber}`, message.from.id);                 
                      storageInfo[userIndex].searchingStep = 0;
                      writeFile(storageInfo, `AllPpl`);
                    }
                  }
